<?php 
	...
	//piechart
	$qry1="select categories,sum(price) as amount from $tablename where entrydate = '$entrydate' GROUP by categories";
	//echo $qry1;
	$result1= $conn->query($qry1);

	//barchart
	$qry2="select categories,sum(price) as amount from $tablename where entrydate = '$entrydate' GROUP by categories";
	//echo $qry1;
	...
	<link rel="stylesheet" type="text/css" href="radiobutton.css">
	 ...
	 <script type="text/javascript">
	 	...
	 	function drawChart1()
	 	{
			...
			var options = {
				 title: 'Daily :Category Wise Expences',
	 	...
	 	function drawChart2()
	 	{
			...
			?>
		 	...
		 	var barchart_options = {title:'Daily :Category Wise Expences',
                        backgroundColor: { fill:'transparent'},
				 		legendTextStyle: { color: '#CCC' },
    			 		titleTextStyle: { color: '#CCC' },
    			 		};
...
<TABLE style="padding-left: 30px; padding-top: 30px;">
	...
	<?php
	$qry="select categories,sum(price) as Amount from $tablename where entrydate = '$entrydate' GROUP by categories";
	//echo $qry;
...
<TABLE style="padding-left: 30px; padding-top: 30px;">
	...
	<?php
	$qry="select * from $tablename where entrydate = '$entrydate' ORDER by categories";
	//echo $qry;
	$result= mysqli_query($conn,$qry);
	while ($row = $result->fetch_assoc())
	 {
