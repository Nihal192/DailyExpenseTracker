<?php 
	...
	$query_date_first = $year . "-01-01";
	$query_date_last = $year . "-12-01";
	$firstdate = date('Y-m-01', strtotime($query_date_first));
	$lastdate = date('Y-m-t', strtotime($query_date_last));
	//echo "<BR>" . $firstdate;
	//echo "<BR>" . $lastdate;
	...
	<link rel="stylesheet" type="text/css" href="radiobutton.css">
	 ...
	 <script type="text/javascript">
	 	...
	 	function drawChart1()
	 	{
			...
			var options = {
				 title: 'Yearly: Category Wise Expences',
	 	...
	 	function drawChart2()
	 	{
			...
			?>
		 	...
		 	var barchart_options = {title:'Yearly: Category Wise Expences',
                        backgroundColor: { fill:'transparent'},
				 		legendTextStyle: { color: '#ccc' },
    			 		titleTextStyle: { color: '#ccc' },
    			 		};
...
<TABLE style="padding-left: 30px; padding-top: 30px;">
	...
	<?php
	$qry="select categories,sum(price) as Amount from $tablename where entrydate >= '$firstdate' and entrydate <='$lastdate' GROUP by categories";
	//echo $qry;
...
<TABLE style="padding-left: 30px; padding-top: 30px;">
	...
	<?php
	$qry="select * from $tablename where entrydate >= '$firstdate' and entrydate <='$lastdate' ORDER by categories";
	//echo $qry;
	$result= mysqli_query($conn,$qry);
	while ($row = $result->fetch_assoc())
	 {
